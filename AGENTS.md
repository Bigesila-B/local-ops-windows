...
本地服务监控与快速启动控制台。**零依赖**：Python 3 标准库后端（单文件）+ 无构建原生前端。推荐双击 `start.bat` 运行；也可用 `python server.py` 调试。

## 结构

- `server.py` — 后端（单文件，仅标准库，Python 3.11）
...
- `static/fonts/GeistMono-Variable.woff2` — vendored 数据/代码字体；中文与正文使用 Windows 系统字体栈；`static/icons/*.svg` — Lucide 图标源文件（vendored）；`tools/gen_icons.py` — 由 svg 重新生成 `icons.js`（勿手改 icons.js）
...
- `%APPDATA%\总控台\config.json` — 用户配置；`icons/` 为应用图标。目录/ 文件权限分别为 0700/0600
- `%APPDATA%\总控台\Logs\{appId}.log` — 应用启动日志；`console.log` 为 `start.bat` 启动日志
- `data/` — 旧版项目内数据，仅在新目标不存在的首次启动中复制迁移；保留不删除
- `start.bat` — Windows 双击启动脚本
- `start.bat` — Windows 启动脚本（启动 `server.py`，输出写入 `%APPDATA%\总控台\Logs\console.log`）

## 运行

`python server.py` → 绑定 `127.0.0.1`，端口从 **9600** 起尝试，被占则 +1（最多 10 个）。启动后自动打开浏览器。`/favicon.ico` 返回统一品牌图标。双击 `start.bat` 会先识别同目录的现有总控台，可直接打开或安全重启，不需要用户输入命令。
...
{
  "services": [{
    "key": "python:8791", "instanceKey": "54252:8791",
    "pid": 54252, "name": "python", "port": 8791,
...
```
- `GET /api/health` — 不运行 `netstat/wmic` 的轻量健康检查，返回 `status/version/schemaVersion/degraded/issues/config`
...
- **进程溯源**：`origin` 沿 PPID 链（≤12 层）识别启动者——跳过壳/包管理器/运行时包装层与系统 init 进程，优先匹配已知 AI 编程助手（codex/claude/kimi/gemini/aider/opencode 等）、GUI 应用（VS Code/Cursor/Windows Terminal 等）、tmux/screen 与总控台 run-token 标记（「总控台」）；未识别的中间层先记为候选、有更优答案即覆盖，全部落空才以最近未识别进程命名；`label` 为展示名、`icon` 取 bot/code/terminal/package/rocket/server，仅用于展示，不影响启停判定

### 服务操作
- `POST /api/kill` `{pid, force?}` → `{ok}` / `{ok:false, error}`（force 用 taskkill /F（强制终止）；校验属当前用户）
...
- `POST /api/pick` `{what: "dir"|"script"}` → `{ok, path}` / `{ok, canceled:true}`（PowerShell 调用 System.Windows.Forms 弹出 Windows 原生目录/文件选择框；取消不是错误）
...
- `POST /api/apps/{id}/restart` → `{ok, pid}` / `{ok:false, error}`（仅重启 token 校验通过的受管进程；等待旧进程退出后再启动，不自动 taskkill /F）
...
- **端口扫描**：`netstat -ano` 列出 TCP 监听端口，按 `(pid, port)` 去重。netstat 不提供进程名，通过 wmic/PowerShell Get-CimInstance 补充。
- **进程详情**：通过 PowerShell Get-CimInstance Win32_Process 查询 PID、命令行、CPU、内存等；只保留 `user == 当前用户`。
- **cwd**：通过 NtQueryInformationProcess (ctypes) 或 PowerShell 查询进程工作目录。
- **运行时间**：通过 Get-CimInstance Win32_Process 的 CreationDate 计算。
- **分组逻辑**（按优先级）：用户 `promoted` → `mine`；进程名含开发关键词（python node ollama docker 等，见 `DEV_KEYWORDS`，只匹配 name 不匹配 args，避免 VS Code `--ms-enable-electron-run-as-node` 这类误伤）→ `mine`（覆盖下方规则，Ollama/Docker 这类在应用内的守护进程仍算服务）；可执行路径含 `Program Files\`（GUI 应用及其 helper）→ `background`；可执行路径以 `C:\Windows\` 开头的系统进程 → `background`；其余默认 `mine`。`hidden` 仅是标记，照常返回。
- **关注进程**：通过 PowerShell Get-CimInstance 查询，筛选 args 包含关键字的进程，只保留当前用户并排除自身及 netstat/wmic。
...
- **应用启停**：多张卡片可保存相同端口（例如多个默认使用 3000 的项目）；启动前只拒绝失效配置和当时真实被占用的端口。重启先做健康预检，失败时不会先停掉仍工作的旧服务。停止时先校验 token，然后只对该受控进程组发 `taskkill`（安全终止进程树），**绝不按端口杀其他监听者**。服务手动 stop 不记录退出历史；任务自然结束记录四态结果，总控台中止记录 `stopped`。批处理不做"长期服务存活探测"，避免把快速成功误判成失败
...
- **无终端 PATH**：双击 `start.bat` 启动时会继承系统环境变量，但可能不包含用户手动添加的 PATH 项；子应用启动环境需显式补入 `~/.local/bin`、Volta/Bun/pnpm、NVM/fnm 与系统 bin 目录，保证 `node`/`npm`/`pnpm` 等可用。启动 API 短暂探测立即退出，并把日志末行作为明确错误返回。
...
- **运行目录**：默认配置/图标位于 `%APPDATA%\总控台`，日志位于 `%APPDATA%\总控台\Logs`；`CONSOLE_DATA_DIR` / `CONSOLE_LOG_DIR` 可显式覆盖，覆盖时对应目录不自动迁移旧 `data/`。
...
- 深浅色跟随系统 + 手动切换（localStorage `console-theme`）；**单一 UI 主题 Ops 指挥台**（ops.css：深空蓝黑/雾灰双色 + 柔和圆角细边 + 蓝色强调，配合布局 v2 的导航轨/KPI 图标卡/实时动态侧栏；`#themeCss` 整包加载机制保留）；字体 = Windows 系统字体栈 + Geist Mono（数据/代码）；顶栏品牌图标 = `static/assets/brand-mark.png`；UI 零 emoji
