...
- [ ] `/api/health` 不执行 `netstat/wmic`，并与 `/api/state` 一致返回当前 `VERSION`、`schemaVersion` 和降级原因。
...
- [ ] 空配置首次启动能自动创建必要目录和文件。
- [ ] 在 %APPDATA%\总控台 数据和日志目录目标不存在时，旧项目 `data/` 的配置、图标和日志只复制一次，旧文件保留不删除。
- [ ] 已存在的新目标不被旧 `data/` 覆盖；显式设置 `CONSOLE_DATA_DIR` / `CONSOLE_LOG_DIR` 时对应自动迁移不发生。
- [ ] 从上一发布版本的真实备份升级成功，应用、图标、标记和主题未丢失；目录和文件使用最小化权限（仅当前用户可访问）。
...
- [ ] 如果仍是“完整项目目录”交付，README 和安装界面已明确说明 Python 3.11 和目录关系。
- [ ] 缺少或版本不符的 Python 会显示可理解、可操作的错误，不会静默退出。
- [ ] 对外分发包已通过 SmartScreen 验证，必要时已进行 Windows 代码签名。
...
- [ ] 发行包不包含旧 `data/`、任何用户 %APPDATA%\总控台 数据和日志目录数据、`tmp/`、`__pycache__/`、`.DS_Store`、`Thumbs.db`、本地虚拟环境或覆盖率文件。
