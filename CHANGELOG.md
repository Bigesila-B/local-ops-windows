...
- 项目从 macOS 移植到 Windows：使用 netstat/wmic/PowerShell 替代 lsof/ps/osascript，数据目录改用 %APPDATA%，启动方式改为 start.bat，支持 .bat/.cmd/.ps1 脚本类型和 Windows 路径分隔符。
...
- 导航轨与侧栏补齐聚合能力：日志中心弹层（Ctrl+J 呼出，应用与总控台日志目录页，Ctrl+L 为浏览器保留键故用 Ctrl+J）、设置中心弹层（任务完成通知开关、浅色/深色/自动外观、版本与目录信息）、快捷操作部件的批量停止服务（确认后逐个安全停止，绝不按端口结束进程）。
...
- Python 最低版本要求从 3.12 调整为 3.11；系统要求从 macOS 12 改为 Windows 10。
- 数据目录从 ~/Library/Application Support/总控台 改为 %APPDATA%\总控台；日志目录从 ~/Library/Logs/总控台 改为 %APPDATA%\总控台\Logs。
- 启动方式从 总控台.app/start.command 改为 start.bat。
- 默认将配置/图标移至 `%APPDATA%\总控台`，日志移至 `%APPDATA%\总控台\Logs`。新目标不存在时仅首次复制旧 `data/`，不删除原文件。
...
- 移除来源和再分发链路不完整的精简中文字体，改用 Windows 系统字体栈。
...
- Apollo/Candy 双 UI 主题、深浅色模式、命令面板和原生 Windows 文件选择。
- `start.bat` 启动脚本。
